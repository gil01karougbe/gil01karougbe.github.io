...
## Introduction
------------------------------------------------------------------------------------------
[delegate](https://www.vulnlab.com/machines) is a medium rated Windows box from VulnLab. The foothold consists of abusing the guest account to enumerate users and access the SYSVOL share, which contains a script leaking a password. Next, we abuse GenericWrite rights to perform targeted Kerberoasting and obtain credentials for a user who is a member of the Remote Management Users group. The privilege escalation path is straightforward and involves abusing the SeEnableDelegationPrivilege privilege.
...
Our current user N.Thompson holds SeEnableDelegationPrivilege on the DC.
```powershell
whoami /priv
```
...
2. GenericWrite abuse to perform a targeted Kerberoasting attack.
