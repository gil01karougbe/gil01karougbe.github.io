...
## Introduction
------------------------------------------------------------------------------------------
[baby](https://www.vulnlab.com/machines) is an easy rated Windows box from VulnLab. The box is actually a domain controller running Active Directory Domain Services. The foothold consists of abusing anonymous access to the LDAP service, where we can read a default password left in a user's description. By spraying that password we identify a user who hasn't changed it yet. The privilege escalation involves abusing the rights of the Backup Operators group to obtain the machine account's NTLM hash, which can then be used to perform a DCSync attack.
...
## Foothold
The LDAP service allows anonymous access. We can use the ldapsearch tool to query information about users, groups, and other Active Directory objects.
```bash
...
Ivan.Walker
```
From the previous LDAP queries we managed to read a default password from Teresa.Bell's description and build a list of valid users. Let's spray that password and see what sticks.
```bash
nxc smb baby.vl -u users.txt -p 'BabyS********'
```
![password spraying](./assets/img/ctf/vulnlab/baby/baby6.png)

The spray confirms that Caroline.Robinson is still using the default password. Since it has expired, we need to set a new one. This can be done as follows:
```sh
...
pass: NewP@ssw0rd
```
With the new password, let's check what services we can access as Caroline.Robinson. We can PSRemote to the DC and we have read/write access on the C$ share.
```bash
...
Our current user, Caroline.Robinson, is a member of the Backup Operators group.
```powershell
...
## References
[resetting expired passwords](https://www.n00py.io/2021/09/resetting-expired-passwords-remotely/)