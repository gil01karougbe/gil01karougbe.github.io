...
## Introduction
------------------------------------------------------------------------------------------
[sync](https://www.vulnlab.com/machines) is an easy rated Linux box from VulnLab. The machine has 4 open ports: 21, 22, 80, and 873, running vsftpd, SSH, Apache2, and rsync respectively. To gain initial foothold you need to abuse the anonymous access to the shared folder exposed by the rsync service. Within that folder you’ll find a backup of the source code for the app running on port 80. From there, read the user hashes stored in the SQLite database and, by understanding the hashing logic, crack `triss`’s password. The privilege escalation involves cracking `/etc/shadow` passwords found in `/backup` and hijacking a scheduled task running as root.
...
```
Reading this portion of the app code we can identify 2 things:
1. There is an SQL injection — the username from the client side is concatenated directly into the SQL query.
```text
admin' or 1=1-- -
triss' or 1=1-- -
```
...
2. The hashing algorithm is MD5, and a hardcoded secret string concatenated with the username is used as a salt. Knowing this salt gives us everything we need to recover user passwords. I tried cracking it with hashcat and JtR but failed — I guess I was missing something. I then wrote a simple Python script to handle it, and that worked.
...
## Kill Chain Summary
1. Exploit rsync anonymous access.
2. Read the registration code to understand the hashing mechanism and crack triss's password.
3. Use triss's credentials to access the FTP server.
4. triss has write permission on her home directory — abuse this to plant a public key in `.ssh/authorized_keys`.
5. Connect to the box through SSH as triss.
6. Crack the Linux passwords (sa and jennifer) found in `/backup/*.zip`.
7. Abuse the root-scheduled task running `/usr/local/bin/backup.sh` — as `sa`, we have write permission on that script.
8. Note: someone could also use `hydra` to brute-force `sa`'s password since it's very weak, and skip steps 1 through 5 entirely.
